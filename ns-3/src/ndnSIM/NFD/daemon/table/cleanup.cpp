...
void
cleanupOnFaceRemoval(NameTree& nt, Fib& fib, Pit& pit, const Face& face, uint64_t endpointId) // Elidio: , uint64_t endpointId
{
  ...
  for (const name_tree::Entry& nte : nt) {
    ...
    if (fibEntry != nullptr) {
      fib.removeNextHop(*fibEntry, face, endpointId); // Elidio
    }

    for (const auto& pitEntry : nte.getPitEntries()) {
      pit.deleteInOutRecords(pitEntry.get(), face, endpointId); // Elidio: endpointId
    }
