...
public:
  ...
  bool
  hasNextHop(const Face& face, const uint64_t endpointId) const; // Elidio

  bool
  hasNextHop(const uint64_t endpointId) const; // Elidio
...
private:
  ...
  std::pair<NextHopList::iterator, bool>
  addOrUpdateNextHop(Face& face, const uint64_t endpointId, uint64_t cost);// Elidio
  ...
  bool
  removeNextHop(const Face& face, const uint64_t endpointId); // Elidio
  ...
  NextHopList::iterator
  findNextHop(const Face& face, const uint64_t endpointId); // Elidio

  NextHopList::iterator
  findNextHop(const uint64_t endpointId); // Elidio
