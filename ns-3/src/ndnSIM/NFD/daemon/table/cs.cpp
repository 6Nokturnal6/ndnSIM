...
makeDefaultPolicy()
{
  return Policy::create("cromo2");// Elidio: lru
}
...
void
Cs::insert(const Data& data, ndn::util::Nmsi nmsi, bool isUnsolicited)
{
  ...
  // Elidio. Should Grab geotag from LP headers?
  ...
  /**********************/ //Not useful for now
//   std::tuple<double, double, double> pos(9.0, 0.9, 9.9);
//   ndn::util::GeoPosition position(9.0, 0.9, 9.9); // Elidio
  /**********************/

  std::tie(it, isNewEntry) = m_table.emplace(data.shared_from_this(), nmsi, isUnsolicited);
