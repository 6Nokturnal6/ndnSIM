...
Pit::findOrInsert(const Interest& interest, bool allowInsert)
{
  ...
  // std::cout << "(Pit::findOrInsert): Inserted interest in PIT. Prefix: " << interest.getName() << " - Node_" << currentNode->GetId() << std::endl;
...
Pit::erase(Entry* entry, bool canDeleteNte)
{
  ...
  ns3::Ptr<ns3::Node> currentNode = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  // std::cout << "(Pit::erase): erased entry in PIT. Prefix: " << entry->getName() << " - Node_" << currentNode->GetId() << std::endl;
  --m_nItems;
}
...
// Elidio
void
Pit::deleteInOutRecords(Entry* entry, const Face& face, uint64_t endpointId)
{
  BOOST_ASSERT(entry != nullptr);

  entry->deleteInRecord(face, endpointId);
  entry->deleteOutRecord(face, endpointId);// Elidio. Initialized as zer0. Needs to be changed. TODO
