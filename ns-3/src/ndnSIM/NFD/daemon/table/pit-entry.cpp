...
{
  return std::find_if(m_inRecords.begin(), m_inRecords.end(),
                [&face, endpointId] (const InRecord& inRecord) { return (&inRecord.getFace() == &face) && (inRecord.getEndpointId() == endpointId); });
}
...
Entry::insertOrUpdateInRecord(Face& face, const Interest& interest)
{
  ...
  // std::cout << "pit-entry.cpp. Interest name: " << interest.getName() << " Endpoint: " << << std::endl;
...
Entry::insertOrUpdateInRecord(Face& face, uint64_t endpointId, const Interest& interest)
{
  ...
  if (it == m_inRecords.end()) {
    ...
    // std::cout << "XXNode_" << node->GetId() << " - endpointId: " << endpointId << std::endl;
  ...
  it->update(interest);
  ns3::Ptr<ns3::Node> node = ns3::NodeList::GetNode(ns3::Simulator::GetContext());
  // std::cout << "insertOrUpdateInRecord: it->getFace().getId(): " << it->getFace().getId() << " - it->getEndpointId(): " << it->getEndpointId() << " - it->getInterest().getName(): " << it->getInterest().getName() << " - Node_"<< node->GetId() << std::endl; // Ok 20240331
  return it;
}
...
Entry::deleteInRecord(const Face& face, uint64_t endpointId)
{
  auto it = std::find_if(m_inRecords.begin(), m_inRecords.end(),
    [&face, endpointId] (const InRecord& inRecord) { return (&inRecord.getFace() == &face) && (inRecord.getEndpointId() == endpointId); });
  if (it != m_inRecords.end()) {
    // std::cout << "Deleting inRecord: " << it->getFace().getId() << " - it->getEndpointId(): " << it->getEndpointId() << " - it->getInterest().getName(): " << it->getInterest().getName() << std::endl;
    m_inRecords.erase(it);
  }
}
...
{
  return std::find_if(m_outRecords.begin(), m_outRecords.end(),
  [&face, endpointId] (const OutRecord& outRecord) { return (&outRecord.getFace() == &face) && (outRecord.getEndpointId() == endpointId); });
}
