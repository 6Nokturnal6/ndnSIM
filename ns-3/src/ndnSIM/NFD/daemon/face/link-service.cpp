...
LinkService::receiveInterest(const Interest& interest, const EndpointId& endpoint)
{
  ...
  // }
  // else{
  //   std::cout << "[LinkService::receiveInterest]... Interest." << std::endl;
  // }

  ++this->nInInterests;
  // std::cout << "VALORRRR: " << endpoint << " fromId: " << fromId << std::endl;
  afterReceiveInterest(interest, 0);// from 0 to fromId
}
...
LinkService::receiveData(const Data& data, const EndpointId& endpoint)
{
  ...
  // }
  // else{
  //   std::cout << "[LinkService::receiveData]... Data." << std::endl;
  // }

  // std::cout << "Recebido endpoint " << endpoint << "fromId: " << fromId << std::endl;
  afterReceiveData(data, 0);// from 0 to fromId
}
