...
findDuplicateNonce(const pit::Entry& pitEntry, uint32_t nonce, const Face& face)
{
    ...
    if (inRecord.getLastNonce() == nonce) {
      ...
      if(fromId != nullptr){
        ...
        // if (endpoint == nodeId){
          ...
          // std::cout << "Node_" << node->GetId() << " - endpoint: " << endpoint << " inRecord.getLastNonce() = " << inRecord.getLastNonce() << " - inRecord.getInterest(): " << inRecord.getInterest() << "Elidio INTEREST OUT " << std::endl;
        //   return;
        // }
      }
      else{
        // std::cout << "[Forwarder::onIncomingInterest]. fromIdTag NOT set! Default 99, set." << std::endl;
        // NFD_LOG_DEBUG("[algorithm]. fromIdTag NOT set! Default 99, set. Should ABORT");
        endpoint = 99;// TODO
      }

      // static int x = 0;
      if ((&inRecord.getFace() == &face) && (inRecord.getEndpointId() == endpoint)) {// && (inRecord.getEndpointId() == endpoint)
  ...
  for (const pit::OutRecord& outRecord : pitEntry.getOutRecords()) {
    if (outRecord.getLastNonce() == nonce) {
      if ((&outRecord.getFace() == &face)) {// && (outRecord.getEndpointId() == endpoint)
                                         ...
                                         const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  for (auto it = nexthops.begin(); it != nexthops.end(); ++it) {
    ...
    // std::cout << "[findEligibleNextHopWithEarliestOutRecord]: " << it->getEndpointId() << std::endl; //Elidio 20230418
    auto outRecord = pitEntry->getOutRecord(it->getFace(), 0);// Elidio 20230109
                  ...
                  time::steady_clock::TimePoint now)
{
  ...
  if (wantUnused) {
    ...
    // std::cout << "[isNextHopEligible]: " << it->getEndpointId() << std::endl; //Elidio 20230418
    auto outRecord = pitEntry->getOutRecord(outFace, 0); //Elidio 20230109
