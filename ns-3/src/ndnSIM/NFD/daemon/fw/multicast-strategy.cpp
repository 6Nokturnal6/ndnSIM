                                        ...
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  ...
  for (const auto& nexthop : nexthops) {
    ...
    // From Elidio
//       const std::string endereco ("00:ff:ff:ff:55:fd");
    //   auto node = ns3::Simulator::GetContext();
//       Interest interest2(interest); // make a copy of the original interest
// //       interest2.setOrigID(endereco); // TODO
// 	  interest2.setTimeA (2);
// 	  interest2.setTimeD (3);
//     // //   std::cout<<"node "<< node <<" get Interest <getName()> "<< interest.getOrigID() << std::endl;
// 	  std::cout<<" get Interest Speed (x, y) = (" << interest.getTimeA() << " , " << interest.getTimeD() << ")" << std::endl;
    // std::cout <<" get Interest with field value "<< interest.getOrigID() << std::endl;
    
    
    
    
    this->sendInterest(pitEntry, FaceEndpoint(outFace, 0), interest); //Elidio
    NFD_LOG_DEBUG(interest << " from=" << ingress << " pitEntry-to=" << outFace.getId());

    if (suppressResult == RetxSuppressionResult::FORWARD) {
      m_retxSuppression.incrementIntervalForOutRecord(*pitEntry->getOutRecord(outFace, 0));// Elidio 20230109
    }
    ++nEligibleNextHops;
  }
