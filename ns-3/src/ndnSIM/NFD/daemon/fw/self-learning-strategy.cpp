...
{
  auto outRecord = pitEntry->getOutRecord(ingress.face, 0);  // Elidio 20230109
...
// 	  std::cout << "Responding discovery Interest... " << std::endl;
    }
  }
                                        ...
                                        const shared_ptr<pit::Entry>& pitEntry)
{
  for (auto& outFace : this->getFaceTable() | boost::adaptors::reversed) {
    ...
    this->sendInterest(pitEntry, FaceEndpoint(outFace, 0xF), interest);
                                        ...
                                        const fib::NextHopList& nexthops)
{
  for (const auto& nexthop : nexthops) {
    ...
    if ((outFace.getId() == inFace.getId() && outFace.getLinkType() != ndn::nfd::LINK_TYPE_AD_HOC) ||
      ...
      continue;
        }//Elidio -> from 0 to 0xF
    this->sendInterest(pitEntry, FaceEndpoint(outFace, 0xF), interest);
    pitEntry->getOutRecord(outFace, 0)->insertStrategyInfo<OutRecordInfo>().first->isNonDiscoveryInterest = true;  // Elidio 20230109
    NFD_LOG_DEBUG("send non-discovery Interest=" << interest << " from="
                  << inFace.getId() << " to=" << outFace.getId());
	std::cout << "send non-discovery Interest=" << interest << " from="
	<< inFace.getId() << " to=" << outFace.getId() << std::endl;
  }
}
...
//   std::cout << "Send announcement... " << std::endl;
...
// 			  std::cout << "found PrefixAnnouncement=" << pa.getAnnouncedName() << std::endl;
              data.setTag(make_shared<lp::PrefixAnnouncementTag>(lp::PrefixAnnouncementHeader(pa)));// Elidio -> from 0 to 0xD
              this->sendDataToAll(pitEntry, FaceEndpoint(*inFace, 0xD), data);
              this->setExpiryTimer(pitEntry, 0_ms);
            }
            else {
              NFD_LOG_DEBUG("PIT entry or Face no longer exists");
			  std::cout << "PIT entry or Face no longer exists" << std::endl;
            }
          });
        }
    });
...
{
// 	std::cout << "Adding some route...???" << std::endl; //NO...
...
// 		std::cout << "Renew route with result=" << res << std::endl;
      });
  });
}
