...
FibUpdater::computeUpdatesForUnregistration(const RibUpdate& update)
{
  ...
  // std::cout << "FIBUpdater..5.." << std::endl;
  // Name prefix exists
  if (ribIt != m_rib.end()) {
                        ...
                        const FibUpdateFailureCallback& onFailure)
{
  ...
  // std::cout << "Applying " << updates.size() << updateString << " to FIBUpdater" << std::endl;
...
FibUpdater::onUpdateSuccess(const FibUpdate update,
                            ...
                            const FibUpdateFailureCallback& onFailure)
{
  // std::cout << "FIBUpdater..7.." << std::endl;
...
{
  // std::cout << "FIBUpdater..8.." << std::endl;
...
{
  // std::cout << "FIBUpdater..9.." << std::endl;
  ...
  if (it != updates.end()) {
	...
	existingUpdate.endpointId = update.endpointId; //Elidio
  }
  else {
...
{
  // std::cout << "FIBUpdater..10.." << std::endl;
  for (const Route& route : routesToAdd) {
    ...
    if (!entry.hasFaceId(route.faceId)) {
      // Create a record of the inherited route so it can be added to the FIBUpdater.. later
      addInheritedRoute(entry.getName(), route);

	  addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), route.faceId, route.cost, route.endpointId));// route.endpointId
    }
  }
}
...
FibUpdater::addInheritedRoutes(const Name& name, const Rib::RouteSet& routesToAdd,
                               const Route& ignore)
{
  // std::cout << "FIBUpdater..11.." << std::endl;
  for (const Route& route : routesToAdd) {
    if (route.faceId != ignore.faceId) {
      // Create a record of the inherited route so it can be added to the FIBUpdater.. later
      addInheritedRoute(name, route);

	  addFibUpdate(FibUpdate::createAddUpdate(name, route.faceId, route.cost, route.endpointId)); // Elidio
    }
  }
}
...
{
  // std::cout << "FIBUpdater..12.." << std::endl;
...
{
  // std::cout << "FIBUpdater..13.." << std::endl;
  ...
  if (other == nullptr || route.cost <= other->cost) {
    addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), route.faceId, route.cost, route.endpointId));// Elidio
  }
}
...
{
  // std::cout << "FIBUpdater..14.." << std::endl;
  ...
  if (costDidChange) {
    ...
    if (route.cost <= entry.getRouteWithLowestCostByFaceId(route.faceId)->cost) {
      ...
      addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), prevRoute->faceId, prevRoute->cost, prevRoute->endpointId)); // Elidio
    }
...
{
  // std::cout << "FIBUpdater..15.." << std::endl;
  addFibUpdate(FibUpdate::createRemoveUpdate(entry.getName(), route.faceId));
  ...
  if (!entry.hasCapture() && entry.getNRoutes() != 0) {
    ...
    if (it != ancestorRoutes.end()) {
	  ...
	  addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), it->faceId, it->cost, it->endpointId));// Elidio
    }
  }
}
...
{
  // std::cout << "FIBUpdater..16.." << std::endl;
                            ...
                            Rib::Rib::RouteSet routesToRemove)
{
  ...
  for (auto addIt = routesToAdd.begin(); addIt != routesToAdd.end(); ) {
    ...
    if (!entry.hasFaceId(addIt->faceId)) {
	  ...
	  addFibUpdate(FibUpdate::createAddUpdate(entry.getName(), addIt->faceId, addIt->cost, addIt->endpointId));// Elidio
    }
...
{
  // std::cout << "FIBUpdater..17.." << std::endl;
  RibUpdate update;
