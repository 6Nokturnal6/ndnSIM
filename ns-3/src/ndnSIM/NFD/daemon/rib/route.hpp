...
public:
   ...
   */
  Route(const ndn::PrefixAnnouncement& ann, uint64_t faceId, uint64_t endpointId = 222); // Elidio
...
public:
  ...
  uint64_t cost = 0;
  uint64_t endpointId = 9999; //Elidio
...
inline bool //Elidio
compareFaceIdAndOriginAndEndpointId(const Route& lhs, const Route& rhs)
{
	return (lhs.faceId == rhs.faceId && lhs.origin == rhs.origin && lhs.endpointId == rhs.endpointId);
}
//------------------------------
