                          ...
                          const std::function<void(RibUpdateResult)>& done)
{
  ...
  NFD_LOG_INFO("Adding route " << name << " nexthop=" << route.faceId <<
               " origin=" << route.origin << " cost=" << route.cost << " endpointId=" << route.endpointId);
  // std::cout << " (rib-manager.cpp): Adding route " << name << " nexthop=" << route.faceId <<
  // " origin=" << route.origin << " cost=" << route.cost << " endpointId=" << route.endpointId << std::endl; // Elidio
...
// 	  route.endpointId = 1010;//res.getEndpoitId(); // Elidio
      route.origin = ndn::nfd::ROUTE_ORIGIN_APP;
      route.flags = ndn::nfd::ROUTE_FLAG_CHILD_INHERIT;
// 	  std::cout << " (registerTopPrefix - insert Rib) route.faceId: " << route.faceId << std::endl;
...
//   std::cout << " (registerTopPrefix) add top prefix to the dispatcher without prefix registration" << std::endl;
                          ...
                          const ndn::mgmt::CommandContinuation& done)
{
  ...
  route.endpointId = parameters.getEndpointId(); // Elidio
  ...
  // std::cout << "(addRoute) ... route.endpointId " << route.endpointId << std::endl;
                        ...
                        ndn::mgmt::StatusDatasetContext& context)
{
  ...
  for (const auto& kv : m_rib) {
    ...
    for (const Route& route : entry.getRoutes()) {
	  ...
	  r.setEndpointId(route.endpointId);// Elidio
...
void
RibManager::slAnnounce(const ndn::PrefixAnnouncement& pa, uint64_t faceId, /* Elidio add*/
    ...
    [=] (const Data&) {
      ...
      beginAddRoute(pa.getAnnouncedName(), route, nullopt,
        [=] (RibUpdateResult ribRes) {
          ...
          cb(res);
		  // std::cout << "slAnnounce " << pa.getAnnouncedName() << " " << faceId << ": " << res << std::endl;
// 		  cb(res);
        });
    },
                    ...
                    const SlAnnounceCallback& cb)
{
  ...
  beginAddRoute(routeName, route, nullopt,
    [=] (RibUpdateResult ribRes) {
	  ...
	  // std::cout << "slRenew " << name << " " << faceId << ": " << res << " " << routeName << std::endl;
      cb(res);
    });
}
...
RibManager::slFindAnn(const Name& name, const SlFindAnnCallback& cb) const
{
  ...
  // std::cout << "slFindAnn " << name << std::endl;
