...
RttEstimator::GetTypeId(void)
{
  static TypeId tid =
    TypeId("ns3::ndn::RttEstimator")
      .SetParent<Object>()
      .AddAttribute("MaxMultiplier", "Maximum RTO Multiplier", UintegerValue(64),/*Elidio changed from 64 to 10*/
                    MakeUintegerAccessor(&RttEstimator::m_maxMultiplier),
                    MakeUintegerChecker<uint16_t>())
      .AddAttribute("InitialEstimation", "Initial RTT estimation", TimeValue(Seconds(1.0)), /*From 1.0*/
                    MakeTimeAccessor(&RttEstimator::m_initialEstimatedRtt), MakeTimeChecker())
      .AddAttribute("MinRTO", "Minimum retransmit timeout value",
                    TimeValue( /* Elidio changed from 0.2 to 1*/
                      Seconds(1)), // RFC2988 says min RTO=1 sec, but Linux uses 200ms. See
                    ...
                    MakeTimeChecker())
      .AddAttribute("MaxRTO", "Maximum retransmit timeout value", TimeValue(Seconds(60.0)), /*Elidio Changed from 200 to 8.0*/
