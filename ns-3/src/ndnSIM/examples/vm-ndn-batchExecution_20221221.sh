...
traceFile=mobility_20221217_1.tcl
logFile=ns2-mobility-trace_20221217.log
log=log_file

storage_dir="/root/fromVM/OneConsumer"
cd $scriptDir
...
# RngRun=(10 15 20 25)
file_to_run=ndn-simple-wifi3-WithBeacon_20221218
control=0
...
#       cd $wafDir
      RngRun=\"${i}\" ${wafDir}/waf --run "$file_to_run --tmp_dir=$tmp_dir --traceFile=$traceFile --nodeNum=$nodeNum --duration=$duration --logFile=$logFile"
