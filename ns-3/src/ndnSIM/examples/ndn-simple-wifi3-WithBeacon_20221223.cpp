...
bool printToFileFIB = true;//false;//
bool printToFilePIT = true;//true;
bool printToFileCS = true;
bool printToFilePosition = true;
...
// 	shared_ptr<nfd::pit::Entry> pitEntry;
	...
	for (nfd::Pit::const_iterator entry = pit.begin(); entry != pit.end(); entry++){
		...
		if (1/*interest_name.getSubName(0, 1) != "/localhost"*/){
...
printFIBmoreData(Ptr<Node> node, const std::string& Ffile, bool control)
{
	...
	for (nfd::Fib::const_iterator entry = fib.begin(); entry != fib.end(); entry++){
		(*outputStream) << Simulator::Now ().ToDouble (Time::MS) << "\t[" << fib.size() << "]: "
		/*(*outputStream)*/ << entry->getPrefix() << " - (NextHop list size: " << entry->getNextHops().size();
		// bool isFirst = true;
...
//   LogComponentEnable("ndn.simple.wifi2", LOG_LEVEL_INFO);
 ...
 Wifi80211pHelper wifi80211p;// = Wifi80211pHelper::Default (); // Do not load Default (avoid "NonUnicastMode", StringValue ...)
    ...
    wifiPhyHelper.Set ("TxPowerEnd", DoubleValue(12.0));
...
//   mobility.Install (vehicles);
  ...
  uint64_t selectedRSU = (uint64_t) randomNum->GetValue(0, 26); //16 26random RSU positions
  ...
  // ndnHelper.SetDefaultRoutes(true);//20230418 ndnHelper.SetDefaultRoutes(false);

  // int xControl = 0;
  // if (!xControl){
  //   ndnHelper.SetDefaultRoutes(true, 666);//20240417 All endpointId defaults to the RSU;
  //   xControl = false;
  //   std::cout << "True xControl: " << xControl << std::endl;
  // }
  // else{
  //   ndnHelper.SetDefaultRoutes(true, /*666*/rsu.Get(0)->GetId());
  //   std::cout << "False xControl: " << xControl << std::endl;
  // }


  for(auto & it: allNodes){
    ndnHelper.SetDefaultRoutes(true, /*666*/it->GetId()); //it->GetId()
    // std::cout << "it->GetId(): " << it->GetId() << std::endl;
    ndnHelper.Install(it);//vehicles
  }
...
//   ndn::AppHelper consumerHelper("ns3::ndn::ConsumerCbr");
  ...
  // routingHelper.Install(allNodes);

  auto producer1 = vehicles.Get(selectedProducer);
  // routingHelper.AddOrigin("/cromo/test/", producer1);
  ...
  // consumerHelper.SetPrefix("/cromo/test/prefix/consumer/");
  consumerHelper.SetAttribute("Frequency", DoubleValue(5.0));//10.0
  ...
  // consumerHelper_20230520.SetPrefix("/cromo/test/prefix/consumer/");
  ...
  appConsumer_20230520.Start(Seconds(2.5));
  // invokeConsumerCromo = false;
...
/*
  ndn::AppHelper consumerHelper_rsu2("ns3::ndn::ConsumerCRoMo2");
  consumerHelper_rsu.SetPrefix("/cromo/test/prefix/consumer/" + std::to_string(rsu.Get(0)->GetId()) + "/");
  // consumerHelper_rsu2.SetPrefix("/cromo/test/prefix/consumer/");
  // consumerHelper_rsu.SetPrefix("/cromo/beacon");
  consumerHelper_rsu2.SetAttribute("Frequency", DoubleValue(5));// = 1 Interest for each 5 seconds// (1/12.5 = 0.08) - limit of 250m of communication range in the urban environment (max speed = 20m/s)
  auto consumer_rsu2 = rsu.Get(0);

  // invokeConsumerCromo = true;
  auto appConsumer_rsu2 = consumerHelper_rsu2.Install(consumer_rsu2);
  appConsumer_rsu2.Start(Seconds(1.0)); // 5 seconds after the simulation starts*/
...
// Producer on selectedProducer
  ...
  producerHelper.SetAttribute("PayloadSize", StringValue("1024"));//2200
  // auto producer1 = vehicles.Get(selectedProducer);
  ...
  producerHelper2.SetAttribute("PayloadSize", StringValue("1024"));//2200
  ...
  auto printFib = [](Ptr<Node> node) {
    ...
    for (const auto& entry : ndn->getForwarder()->getFib()) {
      ...
      for (auto& nextHop : entry.getNextHops()) {
        ...
        cout << " endpointId: " << nextHop.getEndpointId();
  ...
  // cout << "FIB content on node vehicles.Get(2)" << endl;
  // printFib(vehicles.Get(8));



  // for (const auto& xx: vehicles){
  //   if (xx->GetId() == 8){
  //     static int x = 0;
  //     std::cout << "Node_" << xx->GetId() << " - x-x: " << x++ << std::endl;
  //   }
  // }



  // auto node = ns3::NodeList::GetNode(0);//ns3::Simulator::GetContext()
