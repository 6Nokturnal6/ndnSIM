...
//             << ")" << std::endl;
  ...
  // For Kalman Filter
  (*outputStream) << pos.x << "\t" << pos.y << "\t" << speed.x << "\t" << speed.y << "\t" <<  Simulator::Now().GetSeconds() << std::endl;//Simulator::Now().GetMilliSeconds()
...
//   ndn::StrategyChoiceHelper::Install(vehicles, "/", "/localhost/nfd/strategy/multicast");
  ndn::StrategyChoiceHelper::Install(vehicles, "/cromo", "/localhost/nfd/strategy/mobility-aware");
...
//   ndn::AppHelper consumerHelper("ns3::ndn::ConsumerCbr");
  ...
  // consumerHelper.SetPrefix("/cromo/test/prefix/consumer_"+std::to_string(vehicles.Get(30)->GetId()));
  // consumerHelper.SetAttribute("Frequency", DoubleValue(50.0));//100
  // auto consumer2 = vehicles.Get(30);
  ...
  consumerHelper1.SetAttribute("Frequency", DoubleValue(5.0));
  auto RSU0 = rsu.Get(0);
...
// Producer
  ...
  // auto producer3 = vehicles.Get(9);// 3 to 9
  auto appProducer3 = producerHelper2.Install(vehicles.Get(selectedProducerBeacon));// selectedProducerBeacon
  // auto appProducer3 = producerHelper2.Install(vehicles.Get(8));// selectedProducerBeacon
  ...
  if(printToFileAll == false){ // Only print Info of selected nodes
    ...
    if(printToFileFIB) printFIBmoreData (vehicles.Get(29), tmp_dir + "FIB_Node_" + std::to_string(vehicles.Get(29)->GetId()) + ".txt", false);
    if(printToFilePIT) printPIT (vehicles.Get(29), tmp_dir + "PIT_Node_" + std::to_string(vehicles.Get(29)->GetId()) + ".txt", false);
    if(printToFileCS) printCS (vehicles.Get(29), tmp_dir + "CS_Node_" + std::to_string(vehicles.Get(29)->GetId()) + ".txt", false);
    if(printToFilePosition) showPosition (vehicles.Get(29), deltaTime, tmp_dir + "Mobility_Node_" + std::to_string(vehicles.Get(29)->GetId()) + ".txt", false);// 3 seconds
  }
  else {
    for (uint32_t i = 0; i < nodeNum; i++){// +1 for the RSU
