...
//   Config::SetDefault("ns3::WifiRemoteStationManager::NonUnicastMode", StringValue("OfdmRate24Mbps"));
 ...
 Wifi80211pHelper wifi80211p;// = Wifi80211pHelper::Default (); // Do not load Default (avoid "NonUnicastMode", StringValue ...)
    ...
    wifiPhyHelper.Set ("TxPowerStart", DoubleValue(14.58));//33 - 16.7?
    wifiPhyHelper.Set ("TxPowerEnd", DoubleValue(14.58));
    wifiPhyHelper.Set ("TxPowerLevels", UintegerValue(1));
    wifiPhyHelper.Set ("TxGain", DoubleValue(0));//12? 2 -> 355m
    wifiPhyHelper.Set ("RxGain", DoubleValue(0));//12? 2 -> 355m
...
//   ndn::AppHelper consumerHelper("ns3::ndn::ConsumerCbr");
  ...
  // auto consumer0 = nodes.Get(5);
  // auto appConsumer0 = consumerHelper.Install(consumer0);

  consumerHelper.SetPrefix("/cromo/test/prefix/consumer_"+std::to_string(nodes.Get(10)->GetId()));
  consumerHelper.SetAttribute("Frequency", DoubleValue(50.0));//100
  auto consumer1 = nodes.Get(10);
...
//   Simulator::Schedule (Seconds (0.0), &showPosition, producer, deltaTime); // Schedule to print node velocity status
  ...
  ndn::AppDelayTracer::InstallAll(tmp_dir + "delay-ndn-simple-wifi3-WithBeacon_20221217.txt");
//   ndn::L3AggregateTracer::InstallAll ("aggregate-ndn-simple-wifi.txt", Seconds (2));
  ndn::L3RateTracer::InstallAll (tmp_dir + "rate-ndn-simple-wifi3-WithBeacon_20221217.txt", Seconds (5));
