...
main(int argc, char* argv[])
{
  ...
  p2p.Install(nodes.Get(0), nodes.Get(3)); // Elidio. Connecting consumer and the producer
  ...
  auto app1 = consumerHelper.Install(nodes.Get(0)); // first node: Consumer 0
  auto app2 = consumerHelper.Install(nodes.Get(3)); // third node: Consumer 1

  app2.Start(Seconds(10.0)); // start the consumer 1 app at 10 seconds mark
//   apps.Stop(Seconds(10.0)); // stop the consumer app at 5 seconds mark

  /************/
  const bool control = false;
  //int i = 0;
  printFIBmoreData (nodes.Get(0), "FIB_Consumer0.txt", control);
  printPIT (nodes.Get(0), "PIT_Consumer0.txt", control);
  printCS(nodes.Get(0), "CS_Consumer0.txt", control);

  printFIBmoreData (nodes.Get(3), "FIB_Consumer1.txt", control);
  printPIT (nodes.Get(3), "PIT_Consumer1.txt", control);
  printCS(nodes.Get(3), "CS_Consumer1.txt", control);
...
//   apps.Stop(Seconds(10.0)); // stop the consumer app at 5 seconds mark
  ...
  auto prod = producerHelper.Install(nodes.Get(2)); // last node
  prod.Stop(Seconds(3.0)); // stop the producer app at 10 seconds mark
  /**************/
  	// For producer
