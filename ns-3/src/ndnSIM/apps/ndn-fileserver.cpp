  ...
  static TypeId tid =
    TypeId("ns3::ndn::FileServer")
      ...
      .AddAttribute("PayloadSize", "Virtual payload size for Content packets", UintegerValue(1024),
                    MakeUintegerAccessor(&FileServer::m_virtualPayloadSize),
...
FileServer::StartApplication()
{
  ...
  //
  // m_MTU = m_virtualPayloadSize; //1500; //GetFaceMTU(0);
}
...
FileServer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  else
  {
    ...
    seqNo = seqNo - 1; // the client uses seqNo = 1 as the first one, for the server it's better to start at 0
  }
  ...
  // measure how much overhead this actually is
  ...
  m_maxPayloadSize = m_virtualPayloadSize - diff - 4;
...
FileServer::ReturnManifestData(shared_ptr<const Interest> interest, std::string& fname)
{
  ...
  data->setOrigID(nodeMAC);//std::get<0>(*it)//
  ...
  signature.setValue(::ndn::encoding::makeNonNegativeIntegerBlock(::ndn::tlv::SignatureValue, m_signature));
...
FileServer::ReturnPayloadData(shared_ptr<const Interest> interest, std::string& fname, uint32_t seqNo)
{
  ...
  shared_ptr< ::ndn::Buffer> buffer = make_shared< ::ndn::Buffer>(m_maxPayloadSize);
  //size_t actualSize = fread(buffer->get(), sizeof(uint8_t), m_maxPayloadSize, fp);
  fread(buffer->data(), sizeof(uint8_t), m_maxPayloadSize, fp);
  fclose(fp);
...
FileServer::EstimateOverhead(std::string& fname)
{
  ...
  int estimatedMaxPayloadSize = m_virtualPayloadSize - interestLength - 30; // the -30 is something we saw in results, it's just to estimate...
