...
#define MINIMUM_TIMEOUT 8.0 //10.0
...
FileConsumer::GetTypeId(void)
{
  static TypeId tid =
    TypeId("ns3::ndn::FileConsumer")
      ...
      .AddAttribute("LifeTime", "LifeTime for Interests", StringValue("8s"),
      ...
      .AddAttribute("MaxEstimatedRTT", "The maximum RTT the RTTEstimator should have (in ms)", UintegerValue(500), /*500*///8000//1000 changed
                    MakeUintegerAccessor(&FileConsumer::m_maxRTT),
                    MakeUintegerChecker<uint32_t>())
      .AddAttribute("InitialRTT", "The initial RTT the RTTEstimator should have (in ms)", UintegerValue(500), /*500*///8000//1000 changed
      ...
      .AddTraceSource("LastRetransmittedInterestDataDelay",
                      ...
                      MakeTraceSourceAccessor(&FileConsumer::m_lastRetransmittedInterestDataDelay),
                      "ns3::ndn::FileConsumer::LastRetransmittedInterestDataDelayCallback")

      .AddTraceSource("FirstInterestDataDelay",
                      "Delay between first transmitted Interest and received Data",
                      MakeTraceSourceAccessor(&FileConsumer::m_firstInterestDataDelay),
                      "ns3::ndn::FileConsumer::FirstInterestDataDelayCallback");
    ;
...
FileConsumer::FileConsumer()
{
  ...
  if (it != NodeIdMacMap.end()) {
    nodeMAC = std::get<0>(*it);
  }


  m_localDataCache = NULL;
  NS_LOG_INFO("> m_interestLifeTime: " << m_interestLifeTime);
}
...
FileConsumer::SendManifestPacket()
{
  ...
  interest->setOrigID(nodeMAC);

  m_manifestRequestTime = Simulator::Now().GetMilliSeconds();

  // set the interest lifetime
  long timeout = 1.0 * EstimatedRTT + 4.0 * DeviationRTT;
  ...
  NS_LOG_INFO("> Creating INTEREST for " << interest->getName() << " m_interestLifeTime: " << m_interestLifeTime);
...
FileConsumer::SendFilePacket()
{
  ...
  if (m_sequenceStatus[seq] == TimedOut){
    ...
    // m_retxSeqs.erase(m_retxSeqs.begin());// TODO. SEE this
  }
  ...
  NS_LOG_INFO("> File INTEREST (Seq: " << seq << "): " << interest->getName() << " m_interestLifeTime: " << m_interestLifeTime);
...
FileConsumer::CheckSeqForTimeout(uint32_t seqNo)
{
  ...
  if (m_sequenceStatus[seqNo] != Received)
  {
    ...
    // EstimatedRTT = EstimatedRTT * 2; // Uncommented gives long periods for retransmittions ending up finishing simulation before retrieve the file
...
FileConsumer::OnData(shared_ptr<const Data> data)
{
  ...
  SeqTimeoutsContainer::iterator entry = m_seqLastDelay.find(seqNo);
  if (entry != m_seqLastDelay.end()) {
    m_lastRetransmittedInterestDataDelay(this, seqNo, Simulator::Now() - entry->time, hopCount);
  }

  entry = m_seqFullDelay.find(seqNo);
  if (entry != m_seqFullDelay.end()) {
    m_firstInterestDataDelay(this, seqNo, Simulator::Now() - entry->time, m_seqRetxCounts[seqNo], hopCount);
  }

  m_seqRetxCounts.erase(seqNo);
  m_seqFullDelay.erase(seqNo);
  m_seqLastDelay.erase(seqNo);

  m_seqTimeouts.erase(seqNo);
  m_retxSeqs.erase(seqNo);

  // m_rtt->AckSeq(SequenceNumber32(seqNo));
  ...
  NS_LOG_DEBUG("Content Size: " << data->getContent().value_size());
...
FileConsumer::OnFileReceived(unsigned status, unsigned length)
{
  ...
  // do nothing here
  NS_LOG_DEBUG("File downloading finished!");
  ...
  NS_LOG_DEBUG("Elapsed time: " << (_finished_time - _start_time) << "ms; Average Speed: " << downloadSpeed << " b/s. " << " # requests: " << m_curSeqNo << ".");
...
void
FileConsumer::WillSendOutInterest(uint32_t sequenceNumber)
{
  ...
  // static int count = 0;
  // NS_LOG_DEBUG("count: " << ++count << " m_seqTimeouts: " << m_seqTimeouts.size() << " m_seqFullDelay: " << m_seqFullDelay.size() << " m_seqRetxCounts: " << m_seqRetxCounts.size());
