...
#include "ndn-consumer-cromo2BeaconAll.hpp"
#include "ns3/ptr.h"
#include "ns3/log.h"
...
uint64_t sendingInterval = 10; // in Seconds

NS_LOG_COMPONENT_DEFINE("ndn.ConsumerCRoMo2BeaconAll");

namespace ns3 {
namespace ndn {

NS_OBJECT_ENSURE_REGISTERED(ConsumerCRoMo2BeaconAll);

TypeId
ConsumerCRoMo2BeaconAll::GetTypeId(void)
{
  static TypeId tid =
    TypeId("ns3::ndn::ConsumerCRoMo2BeaconAll")
      .SetGroupName("Ndn")
      .SetParent<Consumer>()
      .AddConstructor<ConsumerCRoMo2BeaconAll>()

      .AddAttribute("Frequency", "Frequency of interest packets", StringValue("1.0"),
                    MakeDoubleAccessor(&ConsumerCRoMo2BeaconAll::m_frequency), MakeDoubleChecker<double>())

      .AddAttribute("Randomize",
                    "Type of send time randomization: none (default), uniform, exponential",
                    StringValue("none"),
                    MakeStringAccessor(&ConsumerCRoMo2BeaconAll::SetRandomize, &ConsumerCRoMo2BeaconAll::GetRandomize),
                    MakeStringChecker())

      .AddAttribute("MaxSeq", "Maximum sequence number to request",
                    IntegerValue(std::numeric_limits<uint32_t>::max()),
                    MakeIntegerAccessor(&ConsumerCRoMo2BeaconAll::m_seqMax), MakeIntegerChecker<uint32_t>())
...
ConsumerCRoMo2BeaconAll::ConsumerCRoMo2BeaconAll()
  : m_frequency(1.0)
  , m_firstTime(true)
{
  NS_LOG_FUNCTION_NOARGS();
  m_seqMax = std::numeric_limits<uint32_t>::max();
  // std::cout << "Constructor ConsumerCRoMo2BeaconAll: " << "tmp_dir: " << tmp_dir << " nodeNum:" << nodeNum << std::endl;


  m_nodeConsumerTimer = ns3::Timer(ns3::Timer::CANCEL_ON_DESTROY);
  m_nodeConsumerTimer.SetFunction(&ConsumerCRoMo2BeaconAll::ClearToScheduleNextPacket, this);
  m_nodeConsumerTimer.Schedule(ns3::Seconds(sendingInterval));
...
void
ConsumerCRoMo2BeaconAll::ClearToScheduleNextPacket(){
  clearToSend = true;
  // m_nodeConsumerTimer->SetFunction(&ConsumerCRoMo2BeaconAll::ClearToScheduleNextPacket, this);
  // m_nodeConsumerTimer->Schedule(ns3::MilliSeconds(sendingInterval));
}
...
// Application Methods
void
ConsumerCRoMo2BeaconAll::StartApplication() // Called at time specified by Start
{
  NS_LOG_FUNCTION_NOARGS();
  App::StartApplication();

  shared_ptr<ns3::Timer> m_nodeConsumerTimer;
...
void
ConsumerCRoMo2BeaconAll::ScheduleNextPacket()
{
  ...
  if (1){//clearToSend
    if (m_firstTime) {
      // m_sendEvent = Simulator::Schedule(Seconds(0.0), &ConsumerCRoMo2BeaconAll::SendPacket2, this);
      m_sendEvent = Simulator::Schedule(Seconds(0.0), &ConsumerCRoMo2BeaconAll::SendBeaconAll, this);
      m_firstTime = true; //false; -- Always this event
    }
    else if (!m_sendEvent.IsRunning())
      m_sendEvent = Simulator::Schedule((m_random == 0) ? Seconds(1.0 / m_frequency)
                                                        : Seconds(m_random->GetValue()),
                                        &ConsumerCRoMo2BeaconAll::SendBeaconAll, this);

    clearToSend = false;
    // if(m_nodeConsumerTimer.IsRunning() == false){
    //   m_nodeConsumerTimer.SetFunction(&ConsumerCRoMo2BeaconAll::ClearToScheduleNextPacket, this);
    //   m_nodeConsumerTimer.Schedule(ns3::Seconds(sendingInterval));
    // }
    // ConsumerCRoMo2BeaconAll::ClearToScheduleNextPacket();
...
void
ConsumerCRoMo2BeaconAll::SetRandomize(const std::string& value)
{
  if (value == "uniform") {
...
/******************** Elidio - For Beacon ***********************/
void
ConsumerCRoMo2BeaconAll::SendBeaconAll()
{
  static int invokationsBeaconAll = 0;
  if (!m_active)
    return;

  if (!invokeConsumerClient){
    // std::cout << "NodeId: " << GetNode()->GetId() << " invokeConsumerClient: " << invokeConsumerClient << " exiting" << std::endl;
    return; // first time the method is called from the scenario, it does not send an Interest
  }

  // std::cout << "NodeId: " << GetNode()->GetId() << " [SendBeaconAll] invokation: #" << invokationsBeaconAll++ << " - invokeConsumerClient: " << invokeConsumerClient << std::endl;
  // std::cout << "NodeId: " << GetNode()->GetId() << " [SendBeaconAll] - " << ns3::Simulator::Now().GetMilliSeconds() << ": #" << invokationsBeaconAll++ << std::endl;

  // NS_LOG_INFO("> Sending....");
  NS_LOG_FUNCTION_NOARGS();

  double retxTimeout = 0.5;
...
//   std::cout << "ndn.ndn-consumer (nodeId): " << nodeId << std::endl;
  ...
  interest->setTag<lp::destIdTag>(make_shared<lp::destIdTag>(666)); // Invalid value. All nodes receiving this packet only use it for updating CCTable

  // m_transmittedInterests(interest, this, m_face);
  m_appLink->onReceiveInterest(*interest);

  static uint64_t NRetransmissionsBeaconAll = 1;
  if(NRetransmissionsBeaconAll % 2 != 0){
    m_firstTime = false;
    invokeConsumerClient = true;
    ScheduleNextPacket();
    // std::cout << "NRetransmissionsBeaconAll++: " << NRetransmissionsBeaconAll++ << std::endl;
  }


  // static uint64_t NRetransmissionsAll = 0;
  // if (NRetransmissionsAll <= 5) {
    // ScheduleNextPacket();
  //   NRetransmissionsAll++;
  // }
...
void
ConsumerCRoMo2BeaconAll::OnData(shared_ptr<const Data> data)
{
  if (!m_active)
    return;
  ...
  std::string file = tmp_dir + "ndn-consumer-cromo2BeaconAll_Node" + std::to_string(nodeId) + ".txt";
...
std::string
ConsumerCRoMo2BeaconAll::GetRandomize() const
{
  return m_randomType;
}
