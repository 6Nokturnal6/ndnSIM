...
/* Elidio */
#include "ns3/ndnSIM/NFD/daemon/table/cs-policy-cromo2.hpp"
  ...
  , m_needSetDefaultRoutes(false)
{
  ...
  /* Elidio */
  m_csPolicies.insert({"nfd::cs::cromo2", [] () { return make_unique<nfd::cs::CRoMo2Policy>(); }});
...
// Elidio 20240417
void
StackHelper::SetDefaultRoutes(bool needSet, uint64_t RsuId)
{
  SetDefaultRoutes(needSet);
  m_RsuId = RsuId;
}
...
void
StackHelper::Update(Ptr<Node> node, bool fromForwarder)
{
  if(fromForwarder){
    // std::cout << "Invoked from the Forwarder! Node_" << node->GetId() << std::endl;
    return;
  }
  if (node->GetObject<L3Protocol>() == 0) {
    Install(node);
...
StackHelper::createAndRegisterFace(Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const
{
  ...
  if (m_needSetDefaultRoutes) {
    ...
    // static int x = 0;
    // std::cout << "Defaults... Node_" << node->GetId() << " - x: " << x++ << std::endl;
    FibHelper::AddRoute(node, m_RsuId, 0, 0, 0.0, 0.0, 0.5, 0.6, "/", face, 0);//std::numeric_limits<int32_t>::max()
    // std::cout << "registered face: " << face->getId() << " - Node_" << node->GetId() << std::endl;
  }
  return face;
}
