...
public:
  ...
  static void
  AddRoute(const std::string& nodeName,
           ...
           const double py,
           const double vx,
           const double vy, const Name& prefix, uint32_t faceId, int32_t metric);
   ...
   *
   * \param node   Node
   * \param neighborNodeId Neighbor Id
   * \param prefix Routing prefix
   * \param face   Face
   * \param metric Routing metric
   */
  static void
  AddRoute(Ptr<Node> node, const uint64_t neighborNodeId, const Name& prefix, shared_ptr<Face> face, int32_t metric);
   ...
   * \param px Geo-position_In_X
   * \param py Geo-position_In_Y
   * \param vx Speed_In_X
   * \param vx Speed_In_Y
   * \param prefix Routing prefix
   * \param face   Face
   ...
   */
  static void
  AddRoute(Ptr<Node> node, const uint64_t neighborNodeId, const Name& prefix, uint32_t faceId, int32_t metric);
   ...
   */
  static void
  RemoveRoute(Ptr<Node> node, const Name& prefix, shared_ptr<Face> face, const uint64_t endpointId);
   ...
   */
  static void
  RemoveRoute(Ptr<Node> node, const Name& prefix, uint32_t faceId, const uint64_t endpointId);
   ...
   */
  static void
  RemoveRoute(const std::string& nodeName, const Name& prefix, uint32_t faceId, const uint64_t endpointId);
   ...
   */
  static void
  RemoveRoute(Ptr<Node> node, const Name& prefix, Ptr<Node> otherNode, const uint64_t endpointId);
   ...
   */
  static void
  RemoveRoute(const std::string& nodeName, const Name& prefix, const std::string& otherNodeName, const uint64_t endpointId);


  static void
  RemoveRoute(Ptr<Node> node,
            const uint64_t neighborNodeId,
            const uint64_t timestamp,
            const uint32_t hops,
            const double px,
            ...
            const double vx,
            const double vy,
            const Name& prefix, shared_ptr<Face> face, const uint64_t endpointId);
...
private:
  ...
  static void
  AddNextHop(const ControlParameters& parameters, Ptr<Node> node, bool justForControl);

  static void
  RemoveNextHop(const ControlParameters& parameters, Ptr<Node> node);

  static void
  RemoveNextHop(const ControlParameters& parameters, Ptr<Node> node, bool justForControl);
};
